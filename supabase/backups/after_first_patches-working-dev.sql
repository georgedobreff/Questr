...
CREATE OR REPLACE FUNCTION "public"."check_notification_update_permissions"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
...
ALTER FUNCTION "public"."check_notification_update_permissions"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."check_pet_update_permissions"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  IF CURRENT_USER IN ('authenticated', 'anon') THEN
...
ALTER FUNCTION "public"."check_pet_update_permissions"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_boss_defeat_rewards"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."handle_boss_defeat_rewards"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."handle_daily_login"() RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
BEGIN
  ...
  -- Get current profile data for THIS user only
  SELECT last_login_at, current_streak, longest_streak, streak_freezes_available
  INTO v_last_login, v_current_streak, v_longest_streak, v_streak_freezes
  FROM public.profiles
  WHERE id = v_user_id;

  -- Calculate days since last login (ignoring time, just dates)
  v_days_diff := DATE_PART('day', v_now - v_last_login)::INTEGER;
  ...
  ELSIF v_days_diff > 1 THEN
    ...
    END IF;
  ELSE
    -- Should not happen (negative days?), but initialize if null
    v_current_streak := 1;
    v_longest_streak := 1;
  END IF;
  ...
  SET 
    ...
    streak_freezes_available = v_streak_freezes
  WHERE id = v_user_id;

  RETURN jsonb_build_object(
    'status', 'updated',
    'current_streak', v_current_streak,
    'streak_frozen', (v_days_diff > 1 AND v_streak_freezes >= 0) -- True if freeze logic ran
  );
END;
$$;


ALTER FUNCTION "public"."handle_daily_login"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."hatch_egg"("user_pet_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  v_new_personality TEXT;
BEGIN
  -- Get pet definition info AND verify ownership
  SELECT p.id, p.personalities
  INTO v_pet_id, v_personalities
  FROM public.user_pets up
  JOIN public.pets p ON up.pet_id = p.id
  WHERE up.id = user_pet_id 
    AND up.is_egg = TRUE
    AND up.user_id = auth.uid(); -- SECURITY CHECK

  IF v_pet_id IS NULL THEN
     RETURN jsonb_build_object('error', 'Egg not found, already hatched, or unauthorized');
  END IF;

  -- Assign random personality if available
  IF jsonb_array_length(v_personalities) > 0 THEN
    v_new_personality := v_personalities->>(floor(random() * jsonb_array_length(v_personalities))::int);
  ELSE
    v_new_personality := 'Neutral';
  END IF;

  -- Update user_pet to hatched state
  UPDATE public.user_pets
  SET 
    is_egg = FALSE,
    personality = v_new_personality,
    incubator_warmth = 0 
  WHERE id = user_pet_id AND user_id = auth.uid(); -- Redundant but safe check

  RETURN jsonb_build_object('status', 'hatched', 'personality', v_new_personality);
END;
$$;


ALTER FUNCTION "public"."hatch_egg"("user_pet_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."incubate_egg"("user_pet_id" "uuid") RETURNS "jsonb"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
  ...
  v_target INTEGER;
BEGIN
  -- SECURITY CHECK: Ensure user owns this pet/egg
  SELECT is_egg, incubator_warmth, hatches_at_warmth
  INTO v_is_egg, v_warmth, v_target
  FROM public.user_pets
  WHERE id = user_pet_id AND user_id = auth.uid(); -- ADDED CHECK

  IF NOT FOUND THEN
    RETURN jsonb_build_object('error', 'Egg not found or does not belong to user');
  END IF;

  IF NOT v_is_egg THEN
    RETURN jsonb_build_object('status', 'not_an_egg');
  END IF;

  -- Increment warmth
  UPDATE public.user_pets
  SET incubator_warmth = incubator_warmth + 1
  WHERE id = user_pet_id AND user_id = auth.uid(); -- Redundant but safe check

  -- Check if ready to hatch
  IF (v_warmth + 1) >= v_target THEN
    RETURN jsonb_build_object('status', 'ready_to_hatch');
  ELSE
    RETURN jsonb_build_object('status', 'incubating', 'progress', (v_warmth + 1), 'target', v_target);
  END IF;
END;
$$;


ALTER FUNCTION "public"."incubate_egg"("user_pet_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."resolve_boss_fight"("p_fight_id" "uuid", "p_status" "public"."boss_fight_status", "p_player_hp" integer, "p_boss_hp" integer) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."resolve_boss_fight"("p_fight_id" "uuid", "p_status" "public"."boss_fight_status", "p_player_hp" integer, "p_boss_hp" integer) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."sync_quest_status"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."sync_quest_status"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."epic_items" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
...
ALTER TABLE "public"."epic_items" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."pets" (
    ...
    "personalities" "jsonb" DEFAULT '[]'::"jsonb",
...
CREATE TABLE IF NOT EXISTS "public"."plans" (
    ...
    "total_estimated_duration_days" integer DEFAULT 0 NOT NULL,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "plan_details" "jsonb",
    "plot" "text",
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "plan_generations_count" integer DEFAULT 0,
    "plan_generations_period_start" timestamp with time zone DEFAULT "now"(),
    "oracle_messages_count" integer DEFAULT 0,
    "oracle_messages_period_start" timestamp with time zone DEFAULT "now"(),
...
ALTER TABLE ONLY "public"."epic_items"
    ADD CONSTRAINT "epic_items_chapter_number_key" UNIQUE ("chapter_number");



ALTER TABLE ONLY "public"."epic_items"
    ADD CONSTRAINT "epic_items_pkey" PRIMARY KEY ("id");
...
CREATE INDEX "idx_boss_fights_status" ON "public"."boss_fights" USING "btree" ("status");



CREATE INDEX "idx_boss_fights_user_plan_module" ON "public"."boss_fights" USING "btree" ("user_id", "plan_id", "module_number");
...
CREATE INDEX "subscriptions_ls_cust_id_idx" ON "public"."subscriptions" USING "btree" ("lemon_squeezy_customer_id");



CREATE INDEX "subscriptions_ls_sub_id_idx" ON "public"."subscriptions" USING "btree" ("lemon_squeezy_subscription_id");
...
CREATE OR REPLACE TRIGGER "protect_notification_content" BEFORE UPDATE ON "public"."notifications" FOR EACH ROW EXECUTE FUNCTION "public"."check_notification_update_permissions"();



CREATE OR REPLACE TRIGGER "protect_pet_stats" BEFORE UPDATE ON "public"."user_pets" FOR EACH ROW EXECUTE FUNCTION "public"."check_pet_update_permissions"();
...
CREATE OR REPLACE TRIGGER "trigger_boss_defeat_rewards" AFTER UPDATE OF "status" ON "public"."boss_fights" FOR EACH ROW EXECUTE FUNCTION "public"."handle_boss_defeat_rewards"();
...
CREATE OR REPLACE TRIGGER "trigger_sync_quest_status" AFTER UPDATE OF "is_completed" ON "public"."tasks" FOR EACH ROW EXECUTE FUNCTION "public"."sync_quest_status"();
...
ALTER TABLE ONLY "public"."plans"
    ADD CONSTRAINT "plans_journey_template_id_fkey" FOREIGN KEY ("journey_template_id") REFERENCES "public"."journey_templates"("id");
...
CREATE POLICY "Allow public read access" ON "public"."epic_items" FOR SELECT USING (true);
...
CREATE POLICY "Users can delete their own 
     notifications." ON "public"."notifications" FOR DELETE USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can view their own adventure state" ON "public"."adventure_states" FOR SELECT USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));
...
REVOKE ALL ON FUNCTION "public"."add_dungeon_keys"("p_user_id" "uuid", "p_amount" integer) FROM PUBLIC;
...
GRANT ALL ON FUNCTION "public"."check_notification_update_permissions"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_notification_update_permissions"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_notification_update_permissions"() TO "service_role";



GRANT ALL ON FUNCTION "public"."check_pet_update_permissions"() TO "anon";
GRANT ALL ON FUNCTION "public"."check_pet_update_permissions"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."check_pet_update_permissions"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."handle_boss_defeat_rewards"() TO "anon";
GRANT ALL ON FUNCTION "public"."handle_boss_defeat_rewards"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."handle_boss_defeat_rewards"() TO "service_role";



GRANT ALL ON FUNCTION "public"."handle_daily_login"() TO "anon";
GRANT ALL ON FUNCTION "public"."handle_daily_login"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."handle_daily_login"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."hatch_egg"("user_pet_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."hatch_egg"("user_pet_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."hatch_egg"("user_pet_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."increment_profile_coins"("p_user_id" "uuid", "p_amount" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."increment_profile_coins"("p_user_id" "uuid", "p_amount" integer) TO "service_role";



GRANT ALL ON FUNCTION "public"."incubate_egg"("user_pet_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."incubate_egg"("user_pet_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."incubate_egg"("user_pet_id" "uuid") TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."notify_pet_mission_completion"() FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."notify_pet_mission_completion"() TO "service_role";



REVOKE ALL ON FUNCTION "public"."notify_pet_status"() FROM PUBLIC;
...
REVOKE ALL ON FUNCTION "public"."purchase_pet_item_internal"("p_user_id" "uuid", "p_pet_item_id" bigint) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."purchase_pet_item_internal"("p_user_id" "uuid", "p_pet_item_id" bigint) TO "service_role";



GRANT ALL ON FUNCTION "public"."resolve_boss_fight"("p_fight_id" "uuid", "p_status" "public"."boss_fight_status", "p_player_hp" integer, "p_boss_hp" integer) TO "anon";
GRANT ALL ON FUNCTION "public"."resolve_boss_fight"("p_fight_id" "uuid", "p_status" "public"."boss_fight_status", "p_player_hp" integer, "p_boss_hp" integer) TO "authenticated";
GRANT ALL ON FUNCTION "public"."resolve_boss_fight"("p_fight_id" "uuid", "p_status" "public"."boss_fight_status", "p_player_hp" integer, "p_boss_hp" integer) TO "service_role";
...
REVOKE ALL ON FUNCTION "public"."spend_pet_energy"("p_pet_id" "uuid", "p_amount" integer) FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."spend_pet_energy"("p_pet_id" "uuid", "p_amount" integer) TO "service_role";



REVOKE ALL ON FUNCTION "public"."sync_pet_energy"("p_pet_id" "uuid") FROM PUBLIC;
GRANT ALL ON FUNCTION "public"."sync_pet_energy"("p_pet_id" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."sync_quest_status"() TO "anon";
GRANT ALL ON FUNCTION "public"."sync_quest_status"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."sync_quest_status"() TO "service_role";
...
GRANT ALL ON TABLE "public"."epic_items" TO "anon";
GRANT ALL ON TABLE "public"."epic_items" TO "authenticated";
GRANT ALL ON TABLE "public"."epic_items" TO "service_role";
